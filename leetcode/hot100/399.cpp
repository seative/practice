...
class Solution {
 public:
  vector<double> calcEquation(vector<vector<string>>& equations,
    ...
    unordered_map<std::string, vector<std::pair<string, double>>> graph;
    vector<double> ans(queries.size(), -1.0);

    for (int i = 0; i < equations.size(); ++i) {
      auto& elem = equations[i];
      graph[elem[0]].emplace_back(elem[1], values[i]);
      graph[elem[1]].emplace_back(elem[0], 1.0 / values[i]);
    }
    ...
    for (int i = 0; i < queries.size(); ++i) {
      ...
      double val = 1.0;
      backTrack(graph, start, end, ans[i], val, visited);
    }
    return ans;
  }
  ...
  void backTrack(
      unordered_map<std::string, vector<std::pair<string, double>>>& graph,
      string& start, string& end, double& real, double& val,
      unordered_map<string, bool>& visited) {
    if (start == end) {
      real = val;
      return;
    ...
    visited[start] = true;
    for (auto& [next, nextVal] : graph[start]) {
      val *= nextVal;
      ...
      backTrack(graph, next, end, real, val, visited);
      val /= nextVal;
    }
  }
};