...
public:
  double findMedianSortedArrays(vector<int> &nums1, vector<int> &nums2) {
    ...
    int midIndex = (length1 + length2) / 2 - 1;

    midIndex /= 2;
    while (midIndex > 1) {

      int nums1Start =
          midIndex < (nums1.size() - 1) ? midIndex : nums1.size() - 1;
      int nums2Start =
          midIndex < (nums1.size() - 1) ? midIndex : nums1.size() - 1;

      // vector<int> newVec =
      //     nums1.at(midIndex) > nums2.at(midIndex) ? vector<int>()
    }
  }
};
...
public:
  int getKthElement(const vector<int> &nums1, const vector<int> &nums2, int k) {
     ...
     * 的元素有 nums2[0 .. k/2-2] 共计 k/2-1 个 取 pivot = min(pivot1,
     * pivot2)，两个数组中小于等于 pivot 的元素共计不会超过 (k/2-1) + (k/2-1) <=
     * k-2 个 这样 pivot 本身最大也只能是第 k-1 小的元素 如果 pivot =
