...
struct ptr_traits<P> : applicable_traits {
  ...
  using target_type = std::remove_pointer_t<decltype(std::to_address(
      std::declval<const P&>()))>;
};
template <class T>
...
concept invocable_dispatch =
    requires { typename D::template invoker<T>; } &&
    is_invoker_well_formed<typename D::template invoker<T>, T, NE, R,
                           Args...>();
template <class D, class P, bool NE, class R, class... Args>
concept invocable_dispatch_ptr =
    invocable_dispatch<D, typename ptr_traits<P>::target_type, NE, R,
                       Args...> ||
...
using func_ptr_t =
    std::conditional_t<NE, R (*)(Args...) noexcept, R (*)(Args...)>;
...
R invocation_dispatcher_ref(const char* self, Args... args) noexcept(
    is_invoker_well_formed<F, typename ptr_traits<P>::target_type, true, R,
                           Args...>()) {
  return invoke_dispatch<F, R>(ptr_traits<P>::dereference(*std::launder(
                                   reinterpret_cast<const P*>(self))),
                               std::forward<Args>(args)...);
}
template <class P, class F, class R, class... Args>
R invocation_dispatcher_ptr(const char* self, Args... args) noexcept(
    is_invoker_well_formed<F, const P, true, R, Args...>()) {
  return invoke_dispatch<F, R>(*std::launder(reinterpret_cast<const P*>(self)),
                               std::forward<Args>(args)...);
}
template <class F, class R, class... Args>
R invocation_dispatcher_void(const char*, Args... args) noexcept(
    is_invoker_well_formed<F, void, true, R, Args...>()) {
  return invoke_dispatch<F, R>(std::forward<Args>(args)...);
}
template <class P>
void copying_dispatcher(char* self, const char* rhs) noexcept(
    has_copyability<P>(constraint_level::nothrow)) {
...
void copying_default_dispatcher(char* self, const char* rhs) noexcept {
  std::uninitialized_copy_n(std::assume_aligned<Align>(rhs), Len,
                            std::assume_aligned<Align>(self));
}
template <class P>
void relocation_dispatcher(char* self, const char* rhs) noexcept(
    has_relocatability<P>(constraint_level::nothrow)) {
...
template <class P>
void destruction_dispatcher(char* self) noexcept(
    has_destructibility<P>(constraint_level::nothrow)) {
...
struct overload_traits_impl : applicable_traits {
  ...
  struct meta_provider {
    ...
    static constexpr func_ptr_t<NE, R, const char*, Args...> get() {
      if constexpr (invocable_dispatch<D, typename ptr_traits<P>::target_type,
                                       NE, R, Args...>) {
        return &invocation_dispatcher_ref<
            P,
            typename D::template invoker<typename ptr_traits<P>::target_type>,
            R, Args...>;
        ...
        return &invocation_dispatcher_void<typename D::template invoker<void>,
                                           R, Args...>;
      }
    }
  };
  struct resolver {
    func_ptr_t<NE, R, Args...> operator()(Args...);
  };
...
template <class T>
struct nullable_traits : inapplicable_traits {};
template <class T>
  requires(requires(const T& cv, T& v) {
    { T{} } noexcept;
...
struct composite_meta : Ms... {
  ...
  constexpr composite_meta() noexcept
...
struct dispatch_traits<D>
    : instantiated_t<dispatch_traits_impl, typename D::overload_types, D> {};
...
struct accessor_helper {
  template <class D>
  using accessor = typename D::template accessor<T>;
  template <class O, class I>
  struct reduction : std::type_identity<O> {};
...
template <class... Ds>
struct default_dispatch_traits {
  using default_dispatch = void;
};
template <class D>
struct default_dispatch_traits<D> {
  using default_dispatch = D;
};
template <class F, class... Ds>
struct facade_traits_impl : inapplicable_traits {};
template <class F, class... Ds>
  requires(dispatch_traits<Ds>::applicable && ...)
struct facade_traits_impl<F, Ds...> : applicable_traits,
                                      default_dispatch_traits<Ds...> {
  using copyability_meta =
      lifetime_meta_t<copyability_meta_provider, F::constraints.copyability>;
  using relocatability_meta = lifetime_meta_t<relocatability_meta_provider,
                                              F::constraints.relocatability>;
  using destructibility_meta = lifetime_meta_t<destructibility_meta_provider,
                                               F::constraints.destructibility>;
  using meta = recursive_reduction_t<
      facade_meta_reduction, composite_meta<>, copyability_meta,
      relocatability_meta, destructibility_meta,
      typename dispatch_traits<Ds>::meta..., typename F::reflection_type>;
  using base =
      recursive_reduction_t<accessor_helper<proxy<F>>::template reduction,
                            composite_accessor<>, Ds...>;
...
struct facade_traits<F>
    : instantiated_t<facade_traits_impl, typename F::dispatch_types, F> {};
...
class proxy : public details::facade_traits<F>::base {
  ...
  static constexpr bool HasNothrowPolyConstructor =
      std::conditional_t<proxiable<P, F>,
                         std::is_nothrow_constructible<P, Args...>,
                         std::false_type>::value;
  template <class P, class... Args>
  static constexpr bool HasPolyConstructor =
      std::conditional_t<proxiable<P, F>, std::is_constructible<P, Args...>,
                         std::false_type>::value;
  ...
  template <class P, class... Args>
  static constexpr bool HasPolyAssignment =
      HasPolyConstructor<P, Args...> && HasDestructor;
  static constexpr bool HasTrivialCopyAssignment =
      HasTrivialCopyConstructor && HasTrivialDestructor;
  static constexpr bool HasNothrowCopyAssignment =
      HasNothrowCopyConstructor && HasNothrowDestructor;
  static constexpr bool HasCopyAssignment =
      HasNothrowCopyAssignment ||
      (HasCopyConstructor && HasMoveConstructor && HasDestructor);
  static constexpr bool HasNothrowMoveAssignment =
      HasNothrowMoveConstructor && HasNothrowDestructor;
 ...
 public:
  ...
  proxy(const proxy&) noexcept
    requires(HasTrivialCopyConstructor)
  = default;
  proxy(const proxy&)
  ...
  template <class P, class... Args>
  explicit proxy(std::in_place_type_t<P>,
                 Args&&... args) noexcept(HasNothrowPolyConstructor<P, Args...>)
  ...
  explicit proxy(
      ...
      Args&&... args) noexcept(HasNothrowPolyConstructor<P,
  ...
  proxy& operator=(const proxy&) noexcept
    requires(HasTrivialCopyAssignment)
  = default;
  ...
  void reset() noexcept(HasNothrowDestructor)
    requires(HasDestructor)
  {
    std::destroy_at(this);
    meta_.reset();
  ...
  template <class P, class U, class... Args>
  P& emplace(std::initializer_list<U> il, Args&&... args) noexcept(
      HasNothrowPolyAssignment<P, std::initializer_list<U>&, Args...>)
...
class inplace_ptr {
 public:
  template <class... Args>
  inplace_ptr(Args&&... args) noexcept(
      std::is_nothrow_constructible_v<T, Args...>)
    requires(std::is_constructible_v<T, Args...>)
      : value_(std::forward<Args>(args)...) {}
  inplace_ptr(const inplace_ptr&) noexcept(
      std::is_nothrow_copy_constructible_v<T>) = default;
  inplace_ptr(inplace_ptr&&) noexcept(std::is_nothrow_move_constructible_v<T>) =
      default;
 ...
 public:
  ...
  compact_ptr(const compact_ptr& rhs)
    requires(std::is_copy_constructible_v<T>)
      : ptr_(rhs.ptr_ == nullptr
                 ? nullptr
                 : allocate<storage>(rhs.ptr_->alloc, rhs.ptr_->alloc,
                                     std::as_const(rhs.ptr_->value))) {}
...
proxy<F> make_proxy_impl(Args&&... args) {
  if constexpr (proxiable<inplace_ptr<T>, F>) {
    ...
    return allocate_proxy_impl<F, T>(std::allocator<T>{},
                                     std::forward<Args>(args)...);
  }
}
...
proxy<F> make_proxy_inplace(Args&&... args) noexcept(
    std::is_nothrow_constructible_v<T, Args...>) {
...
make_proxy_inplace(std::initializer_list<U> il, Args&&... args) noexcept(
  ...
  return proxy<F>{std::in_place_type<details::inplace_ptr<T>>, il,
                  std::forward<Args>(args)...};
}
template <facade F, class T>
proxy<F> make_proxy_inplace(T&& value) noexcept(
...
template <class... Args>
void invalid_call(Args&&...) = delete;
template <class T, class...>
struct dependent : std::type_identity<T> {};
template <class T, class... U>
using dependent_t = typename dependent<T, U...>::type;

template <class O, class I>
struct flat_reduction : std::type_identity<O> {};
...
template <class O, class I>
  requires(is_tuple_like_well_formed<I>())
struct flat_reduction<O, I> : instantiated_t<flat_reduction_impl, I, O> {};

template <class Ds = std::tuple<>,
          proxiable_ptr_constraints C = relocatable_ptr_constraints,
          class R = void>
...
#define ___PRO_DIRECT_FUNC_IMPL(__EXPR) \
  noexcept(noexcept(__EXPR))            \
    requires(requires { __EXPR; })      \
  {                                     \
    return __EXPR;                      \
  }
#define ___PRO_DEF_DISPATCH_IMPL(__NAME, __EXPR, __DEFEXPR, __OVERLOADS)       \
  struct __NAME {                                                              \
   ...
   public:                                                                     \
    ...
    struct accessor {                                                          \
      template <class... __Args>                                               \
      decltype(auto) __NAME(__Args&&... __args) const ___PRO_DIRECT_FUNC_IMPL( \
          (static_cast<::pro::details::dependent_t<const __P*, __Args...>>(    \
               this)                                                           \
               ->template invoke<__Name>(::std::forward<__Args>(__args)...)))  \
    };                                                                         \
  }
#define PRO_DEF_MEMBER_DISPATCH_WITH_DEFAULT(__NAME, __FUNC, __DEFFUNC, ...) \
  ___PRO_DEF_DISPATCH_IMPL(                                                  \
      __NAME, __self.__FUNC(::std::forward<__Args>(__args)...),              \
...
#define PRO_DEF_FREE_DISPATCH_WITH_DEFAULT(__NAME, __FUNC, __DEFFUNC, ...) \
  ___PRO_DEF_DISPATCH_IMPL(                                                \
      __NAME, __FUNC(__self, ::std::forward<__Args>(__args)...),           \
      __DEFFUNC(::std::forward<__Args>(__args)...), ::std::tuple<__VA_ARGS__>)
