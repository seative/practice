...
static void printhex(__m128d m) { printhex(_mm_castpd_si128(m)); }

static void printhex(__m128 m) { printhex(_mm_castps_si128(m)); }

static void print16(__m128i m) {
  printnl("{ ", _mm_cvtsi128_si32(m) & 0xff, ", ", _mm_cvtsi128_si32(m) >> 16,
...
static void print(__m128d m) {
  printnl("{ ", _mm_cvtsd_f64(m), ", ", _mm_cvtsd_f64(_mm_shuffle_pd(m, m, 1)),
          " }\n");
}

static void print(__m128 m) {
  printnl("{ ", _mm_cvtss_f32(m), ", ", _mm_cvtss_f32(_mm_shuffle_ps(m, m, 1)),
...
static void printhex(__m256d m) { printhex(_mm256_castpd_si256(m)); }

static void printhex(__m256 m) { printhex(_mm256_castps_si256(m)); }

static void print(__m256i m) {
  printnl("{ ", _mm256_cvtsi256_si32(m), ", ", _mm256_extract_epi32(m, 1), ", ",
          _mm256_extract_epi32(m, 2), ", ", _mm256_extract_epi32(m, 3), ", ",
          _mm256_extract_epi32(m, 4), ", ", _mm256_extract_epi32(m, 5), ", ",
          _mm256_extract_epi32(m, 6), ", ", _mm256_extract_epi32(m, 7), " }\n");
}
