...
template <class T>
struct _is_printer_printable<
    T, std::void_t<typename _printer<T>::is_default_print>> : std::false_type {
};

template <class T, class = void>
struct is_printable
    : std::disjunction<_is_default_printable<T>, _is_printer_printable<T>> {};
...
struct _enable_if_glmvec<
    ...
    std::enable_if_t<
        ...
        std::is_same_v<decltype(std::declval<typename T::bool_type>().x),
                       bool>>> {
  using type = U;
};
...
struct _enable_if_iterable<
    T, U,
    std::void_t<typename std::iterator_traits<decltype(std::begin(
...
struct _enable_if_has_print<
    T, U,
    std::void_t<decltype(std::declval<T const &>().do_print(
...
struct _enable_if_string<std::basic_string<T, Alloc, Traits>, U,
                         typename _enable_if_char<T>::type> {
  using type = U;
};

template <class T, class Traits, class U>
struct _enable_if_string<std::basic_string_view<T, Traits>, U,
                         typename _enable_if_char<T>::type> {
  using type = U;
};
...
struct _enable_if_c_str<
    ...
    std::enable_if_t<std::is_pointer_v<std::decay_t<T>> &&
                     _is_char<std::remove_const_t<
                         std::remove_pointer_t<std::decay_t<T>>>>::value>> {
  using type = U;
};
...
struct _printer<T, typename _enable_if_has_print<T>::type> {
  static void print(std::ostream &os, T const &t) { t.do_print(os); }
};
...
struct _printer<
    T, typename _enable_if_has_print<
           T, typename _enable_if_iterable<
                  T, typename _enable_if_c_str<
                         T, typename _enable_if_string<
                                T, typename _enable_if_map<T>::not_type>::
...
struct _printer<
    T, typename _enable_if_has_print<
           T, typename _enable_if_tuple<T, typename _enable_if_iterable<
                                               T>::not_type>::type>::not_type> {
  template <std::size_t... Is>
  static void _unrolled_print(std::ostream &os, T const &t,
                              std::index_sequence<Is...>) {
    os << "{";
    ((_printer<_rmcvref_t<std::tuple_element_t<Is, T>>>::print(os,
                                                               std::get<Is>(t)),
      os << ", "),
     ...);
    if constexpr (sizeof...(Is) != 0)
      _printer<_rmcvref_t<std::tuple_element_t<sizeof...(Is), T>>>::print(
          os, std::get<sizeof...(Is)>(t));
    os << "}";
  }
  ...
  static void print(std::ostream &os, T const &t) {
    _unrolled_print(
        ...
        std::make_index_sequence<
            std::max(static_cast<std::size_t>(1), std::tuple_size_v<T>) - 1>{});
  }
};

template <class T>
struct _printer<T, typename _enable_if_has_print<
                       T, typename _enable_if_glmvec<T>::type>::not_type> {
  static void print(std::ostream &os, T const &t) {
    os << "{";
    bool once = false;
...
template <class T>
struct _printer<T, typename _enable_if_has_print<
                       T, typename _enable_if_glmmat<T>::type>::not_type> {
  static void print(std::ostream &os, T const &t) {
    os << "{";
    bool once = false;
    for (typename T::length_type i = 0; i < t.length(); i++) {
...
template <class T>
struct _printer<T, typename _enable_if_has_print<
                       T, typename _enable_if_map<T>::type>::not_type> {
  static void print(std::ostream &os, T const &t) {
    os << "{";
    bool once = false;
...
template <class T>
struct _printer<T, typename _enable_if_has_print<
                       T, typename _enable_if_string<T>::type>::not_type> {
  static void print(std::ostream &os, T const &t) { os << std::quoted(t); }
};
...
template <class T>
struct _printer<T, typename _enable_if_has_print<
                       T, typename _enable_if_optional<T>::type>::not_type> {
  static void print(std::ostream &os, T const &t) {
    if (t.has_value()) {
...
template <class T>
struct _printer<T, typename _enable_if_has_print<
                       T, typename _enable_if_variant<T>::type>::not_type> {
  static void print(std::ostream &os, T const &t) {
    std::visit(
        [&](auto const &v) { _printer<_rmcvref_t<decltype(v)>>::print(os, v); },
        t);
  }
};
